   ...
   const navigate=useNavigate();
    ...
    const generateRecaptha=()=>{
      window.recaptchaVerifier=new RecaptchaVerifier('recaptcha-container',{
        'size':'invisible',
        'callback':(response)=>{
   //reCapthcha solved, allow signInWiththe Phone Number
        }
      },auth);
    }
    const requestOtp=()=>{
    ...
    return (
        <div className='w-full flex flex-col items-center mt-8'>
          ...
          <div class="relative">
              ...
              <input value={OTP} onChange={(e) => setOTP(e.target.value)} id="message" name="message" class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
          </div>
      </div>
      <div class="p-2 w-full md:w-1/3">
